...
export const handleRoute = async (c: ListContext, noCache: boolean) => {
  ...
  const typeConfig = listType[type as keyof typeof listType];
  if (!typeConfig) {
    return {
      code: 400,
      message: `无效的榜单类型: ${type}，可选值: ${Object.keys(listType).join(", ")}`,
    } as unknown as RouterData;
  }
...
const getList = async (options: Options, noCache: boolean) => {
  const { type } = options;
  // 必要数据
  const currentTypeConfig = listType[type as keyof typeof listType];
  if (!currentTypeConfig) {
    return { fromCache: false, updateTime: new Date().toISOString(), data: [] };
  }
  const { params, www } = currentTypeConfig;
