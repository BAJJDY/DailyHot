...
const getList = async (options: Options, noCache: boolean): Promise<RouterResType> => {
  ...
  if (!noCache) {
    ...
    if (cached) {
      logger.info("💾 [CHCHE] bilibili from cache");
      return { fromCache: true, updateTime: cached.updateTime as string, data: cached.data as RouterResType["data"] };
    }
  }
  ...
  const result = await get<BiliResponse>({
    url,
    noCache: true, // 不用 URL 缓存，用上面的稳定 key
    headers: {
  ...
  if (result.data?.data?.list?.length && result.data.data.list.length > 0) {
    ...
    const data = list.map((v) => ({
      id: v.bvid,
      title: v.title,
      desc: (v.desc && v.desc !== "-") ? v.desc : "该视频暂无简介",
    ...
    await setCache(cacheKey, { data, updateTime });
