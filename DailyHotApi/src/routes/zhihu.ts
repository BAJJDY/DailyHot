...
const getList = async (noCache: boolean) => {
  ...
  return {
    ...
    data: list.map((v) => {
      ...
      const detailTextParts = v.detail_text?.split(" ");
      const hotValue = detailTextParts && detailTextParts.length > 0 ? parseFloat(detailTextParts[0]) : 0;
      return {
        id: data.id,
        title: data.title,
        desc: data.excerpt,
        ...
        hot: isNaN(hotValue) ? 0 : hotValue * 10000,
